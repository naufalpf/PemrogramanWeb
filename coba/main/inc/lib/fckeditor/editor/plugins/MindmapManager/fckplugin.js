...
// ================================================
	...
	var FCKMindmapManager = function(name) {
		this.Name = name;
	}
	// manage the plugins' button behavior
	FCKMindmapManager.prototype.GetState = function() {
		return FCK_TRISTATE_OFF;
	}

	FCKCommands.RegisterCommand('MindmapManager', new FCKMindmapManager('MindmapManager')) ;
	 
	// Create the toolbar button.
	var oMindmapManagerItem = new FCKToolbarButton( 'MindmapManager', "MindmapManager", null, null, false, true ) ;

	//oMindmapManagerItem.IconPath = FCKConfig.PluginsPath + 'MindmapManager/icon.gif' ;
//	oMindmapManagerItem.IconPath = FCKConfig.ImagesIcon ; // Use a custom icon.
oMindmapManagerItem.IconPath = FCKConfig.PluginsPath + 'MindmapManager/mindmap.png' ;


	FCKToolbarItems.RegisterItem( 'MindmapManager', oMindmapManagerItem ) ;

	FCKMindmapManager.prototype.Execute = function() {
		MindmapManager_click(FCK, null);
	}

	function MindmapManager_click(editor, sender) {
		...
		//-------------------------------------------------------------------------
		var manager = new MindmapManager();
...
{
	//var tt = MindmapManager.I18N;
};


// Open up the plugin's dialog with manager or editor.
MindmapManager.prototype.insert = function(outparam)
{
	// show image editor
	if (IM_directEdit)
	{
		...
		if (sElm != null && sElm.nodeName.toLowerCase() == 'img')
		{
			...
			var url = FCKConfig.PluginsPath + 'MindmapManager/editor.php?img=' + "/" + imgFileName;
	...
	else
	{
		...
		var manager = FCKConfig.PluginsPath+'MindmapManager/manager.php?base_url_alt='+FCKConfig.CreateDocumentDir;
		...
		//}, outparam);
		}, outparam, 'FCKDialog_MindmapManager', 'Insert Mindmap', 900, 535 );
	}
};
...
{
	if ( FCKConfig.OpenMindmapManagerInANewWindow && FCKConfig.OpenMindmapManagerInANewWindow.toString() == 'true' ) {
