...
// ================================================
	...
	var FCKMascotManager = function(name) {
		this.Name = name;
	}
	// manage the plugins' button behavior
	FCKMascotManager.prototype.GetState = function() {
		return FCK_TRISTATE_OFF;
	}

	FCKCommands.RegisterCommand('MascotManager', new FCKMascotManager('MascotManager')) ;
	 
	// Create the toolbar button.
	var oMascotManagerItem = new FCKToolbarButton( 'MascotManager', "MascotManager", null, null, false, true ) ;

	//oMascotManagerItem.IconPath = FCKConfig.PluginsPath + 'MascotManager/icon.gif' ;
//	oMascotManagerItem.IconPath = FCKConfig.ImagesIcon ; // Use a custom icon.
oMascotManagerItem.IconPath = FCKConfig.PluginsPath + 'MascotManager/mascot.png' ;


	FCKToolbarItems.RegisterItem( 'MascotManager', oMascotManagerItem ) ;

	FCKMascotManager.prototype.Execute = function() {
		MascotManager_click(FCK, null);
	}

	function MascotManager_click(editor, sender) {
...
function MascotManager()
{
	//var tt = MascotManager.I18N;
};


// Open up the plugin's dialog with manager or editor.
MascotManager.prototype.insert = function(outparam)
{
	// show image editor
	if (IM_directEdit)
	{
		...
		if (sElm != null && sElm.nodeName.toLowerCase() == 'img')
		{
			...
			var url = FCKConfig.PluginsPath + 'MascotManager/editor.php?img=' + "/" + imgFileName;
	...
	else
	{
		...
		var manager = FCKConfig.PluginsPath+'MascotManager/manager.php?base_url_alt='+FCKConfig.CreateDocumentDir;
		...
		//}, outparam);
		}, outparam, 'FCKDialog_MascotManager', 'Insert Mascot', 900, 535 );
	}
};
...
{
	if ( FCKConfig.OpenMascotManagerInANewWindow && FCKConfig.OpenMascotManagerInANewWindow.toString() == 'true' ) {
